...
export const getDBInstance = async (
	dataPath: string,
	dbName = 'lisk-framework-scmData-plugin.db',
): Promise<KVStore> => {
...
export const setEventSCMData = async (
	db: KVStore,
	_lskAddress: Buffer,
	_date: string,
    _component: string,
    _demand: string,
    _demandForecast: string,
	_invTurnover: string,
	_invTurnoverForecast: string,
	_eventHeight: number,
    lastCounter: number	
): Promise<void> => {
	const encodedAddressInfo = codec.encode(offChainEventSchema, {
		...
		invTurnover: _invTurnover,
		invTurnoverForecast: _invTurnoverForecast,        
		height: _eventHeight,
	});
...
export const getEventSCMData = async (db: KVStore): Promise<(Event & { id: Buffer })[]> => {
	...
	const results = await new Promise<(Event & { id: Buffer })[]>((resolve, reject) => {
		...
		stream
			.on('data', ({ key, value }: { key: Buffer; value: Buffer }) => {
				...
				} catch (e) { console.error('Codec Error:', e); }
			})
			.on('error', error => {
				reject(error);
			})
...
// Stores height of block where scmData event exists.
...
// Returns height of block where scmData event exists.
